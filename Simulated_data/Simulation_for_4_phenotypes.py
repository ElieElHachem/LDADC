...
from Function_for_generate_data_and_LDADC import generate_multiple_probability,test_blob_left_right_proba,relabel_matrix,continuous_data_files_concat_kmeans_reattached_V2, storage_position,LDA_function_makeblop_form,  number_of_phenotypes_to_test_with_vector
...
for dim in tqdm(dimension_to_test, leave = True ,desc='Dimension'):
    clusters_to_test = []
    clusters_to_test.append(2**dim)
    for cluters in tqdm(clusters_to_test, leave = False,desc='Cluster'):
        compiled_information = {'Dimension':[] ,'Number of Phenotypes':[],'Vectors of probability':[],'Std value': [],'Accuracy':[]}
        proba_to_test = generate_multiple_probability(dimension=dim,first_array=proba_first_array,c=pats,phenotypes=n_pheno)

        for value in tqdm(test_std, desc='STD value processed',leave = False): 
            ...
            dataframe_compiled.to_csv(path_to_store_frame + f'/Compiled_information_for_{dim}_dim_tested_multiple_dim_subseted_{n_pheno}_phenotypes.csv')
            euc_dist_list = []
            for i in dataframe_compiled['Vectors of probability']:
                nbrs = NearestNeighbors(n_neighbors=n_pheno, algorithm='ball_tree', metric='euclidean').fit(i)
                distances, indices = nbrs.kneighbors(i)
                euc_dist = distances[0][1]
                euc_dist_list.append(euc_dist)

            dataframe_compiled['Euclidean Distance'] = euc_dist_list
            dataframe_compiled['Euclidean Distance'] = dataframe_compiled['Euclidean Distance'].round(5)
        ...
        plot_average = ggplot(df_for_average, aes(x='Euclidean Distance', y='Accuracy')) +\
            ...
            scale_y_continuous(limits=(0.3, 1)) +\
