...
def LDA_function_makeblop_form(dataframe_for_LDA,diviser_of_matrix,runrun,number_of_calculation, alpha, beta):
    ...
    norm_phi = phi_z_w.T.copy()
    ns_phi = np.sum(norm_phi, axis=1)
    for i in range(ns_phi.shape[0]):
        #if norm_phi[i] > 0:
        norm_phi[i, :] /= ns_phi[i]

    return np.around(norm_theta_max) , norm_phi 
...
if __name__ == '__main__':
    ...
    microbiota_test = ask_yesno('Do you want to test it on microbiota ?' )
    ...
    for calculation in tqdm(range(time_of_calculation),leave=False,desc='Iteration'):
        ...
        norm_theta, norm_phi = LDA_function_makeblop_form(dataframe_for_LDA=data_frame_for_LDA,diviser_of_matrix = div_mat,runrun = phenotypes_patients,number_of_calculation=number_of_calculation, alpha = 1/number_of_calculation, beta = 1/number_of_calculation)
        labelList = data_frame_for_LDA.T.columns.to_list()
        label_bacteria = data_frame_for_LDA.columns.to_list()
        frame_binary_with_threshold = pd.DataFrame(norm_theta,index=labelList,columns= [f'Topic {i}' for i in range(phenotypes_patients)])
    
        dataframe_norm_pheno = pd.DataFrame(norm_phi,index = label_bacteria, columns= [f'Topic {i}' for i in range(phenotypes_patients)])
        list_of_phi_frame.append(dataframe_norm_pheno)
        dataframe_norm_pheno.dropna(axis=0,inplace=True)

        if microbiota_test == False:
            dataframe_norm_pheno.sort_values(by = 'Topic 0', ascending = False,inplace=True)
        ...
        fig.savefig(path_to_store_figures + f'/Proba_word_topic_bacteria_{phenotypes_patients}_topics_same_nbr_healthy.svg', dpi = 1200)
...
#### ATTENTION !!!!!!!!!!!!!! INSPECT THE TYPE OF TOPIC !!!!!! (see frame_grouped_ungrouped and test  (# frame_grouped_ungrouped['Patient Statut'].replace({2: 0}, inplace=True)) )

# from turtle import color
# test = pd.concat(list_of_dataframe)
# grouped_frame = pd.DataFrame(test.groupby(['Health status','Patient Statut']).count()['Individual'])
# frame_grouped_ungrouped = pd.DataFrame(grouped_frame.reset_index())
# frame_grouped_ungrouped['Patient Statut'].replace({2: 0}, inplace=True)
# plot_order = frame_grouped_ungrouped.groupby('Patient Statut')['Individual'].sum().sort_values(ascending=False).index.values
# g = sns.catplot( y="Individual", x='Patient Statut', data=frame_grouped_ungrouped,hue ='Health status', kind="bar", ci='sd', capsize=.2, order=plot_order, palette=['#D92B04','#45858C'], legend=False)
# g.set_xlabels('Topic', fontsize=20) # not set_label
# g.set_ylabels('Number of patients', fontsize=20) # not set_label
# plt.legend(loc = 'upper right',title = 'Condition', title_fontsize = 15, bbox_to_anchor=(1, 1), ncol = 1, prop = {'size' : 13})
#plt.savefig(path_to_store_figures + f'/Type_of_patient_inside_each_topic_{phenotypes_patients}_topic.svg', dpi = 1200)
